...
class ESSearch:
    ...
    def search_dialog_by_call_id(self, call_id):
        ...
        for i in res['hits']['hits']:
            i['_source']['end_time']=i['_source']['end_time'][:-5].replace('T',' ')
            i['_source']['begin_time']=i['_source']['begin_time'][:-5].replace('T',' ')
    ...
    def insertES(self,id_num,dialog_id,transcripts,begin_time,end_time,silence_max,silence_total,call_id,emotion,interruption,session_id,topic,seven_type_analyzer,necessary_keypoint_analyzer,id_value=None):
        '''
        descreption:新建索引条目，须填入全部字段
        params:(见下方注释)
        return: None
        '''
        body={}
        body['id']=id_num                              #<string>
        ...
        body['seven_type_analyzer']=seven_type_analyzer                   #<string>  
        body['necessary_keypoint_analyzer']=necessary_keypoint_analyzer     #<string> 
        self.es.index(index=self.index, doc_type=self.doc_type,id=id_value,body=body)
    def search_dialog_by_interuption(self, interruption=True):     
        ...
        ''' 
        body = {'query':{'match_phrase':{'interruption':{'query':interruption}}}}
        res = self.es.search(index=self.index, body=body)
        return_list=[]
        return_dict={}
        for i in res['hits']['hits']:
    ...
    def search_dialog_by_seven_point_analyzer(self, seven_type_analyzer):
        '''
        descreption:检索seven_point_analyzer字段，即’辱骂‘，’用心超越希望‘等七类
        params: seven_type_analyzer:<str> ,topic:<str>
        return: [{"dialog_id":<str>,"transcripts",<str>,"seven_type_analyzer": <str>(e.g. 辱骂: True...)}]
        '''  
        body = {'query':{'match_phrase':{'seven_type_analyzer':{'query':seven_type_analyzer+': True'}}}}
        res = self.es.search(index=self.index, body=body)
        return_list=[]
        return_dict={}
        for i in res['hits']['hits']:
            return_dict["dialog_id"]=i['_source']['dialog_id']
            return_dict["transcripts"]=i['_source']['transcripts']
            return_dict['seven_type_analyzer']=i['_source']['seven_type_analyzer']
            return_list.append(return_dict)
        return return_list
    def search_dialog_by_necessary_keypoint_analyzer(self, keypoint_list,topic):
        '''
        descreption:检索necessary_keypoint_analyzer字段，即对话关键点
        params: keypoint_list:[keypoint1<str>,keypoint2<str>...] ,topic:<str>
        ...
        for i in keypoint_list:            
            keypoint.append({'match_phrase':{'necessary_keypoint_analyzer':{'query':i+': True'}}})        
        ...
        for i in res['hits']['hits']:
            return_dict["dialog_id"]=i['_source']['dialog_id']
            return_dict["transcripts"]=i['_source']['transcripts']
            return_dict["key_point_status"]=i['_source']['necessary_keypoint_analyzer']
        return_list.append(return_dict)
        return return_list
    def updateES(self,id_num,dialog_id,transcripts,begin_time,end_time,silence_max,silence_total,call_id,emotion,interruption,session_id,topic,seven_type_analyzer,necessary_keypoint_analyzer):
        '''
        descreption:更新索引条目，须填入全部字段
        params:(见下方注释)
        return: None
        '''
        body1 = {'query':{'match_phrase':{'dialog_id':{'query':dialog_id}}}}
        res = self.es.search(index=self.index, body=body1)
        ...
        body['silence_max']=silence_max  
        body['silence_total']=silence_total    
        body['call_id']=call_id    
        body['emotion']=emotion    
        body['interruption']=interruption    
        body['session_id']=session_id    
        body['topic']=topic
        body['seven_type_analyzer']=seven_type_analyzer
        body['necessary_keypoint_analyzer']=necessary_keypoint_analyzer
        self.es.index(index=self.index, doc_type=self.doc_type,id=id_value,body=body)
...
#    ESSearch.insertES(id_num=1,dialog_id=2,transcripts='asdfghjkl',begin_time='2018-01-01T00:00:00.000Z',end_time='2018-01-02T00:00:00.000Z',silence_max=10.01,silence_total=123.456,call_id=1,emotion='消极',interruption=True,session_id='1',topic='现金业务',seven_type_analyzer="辱骂: True",necessary_keypoint_analyzer="a: True, b: True, c: True")
    ESSearch.updateES(id_num=1,dialog_id=2,transcripts=transcripts,begin_time='2018-01-01T00:00:00.000Z',end_time='2018-01-02T00:00:00.000Z',silence_max=10.01,silence_total=123.456,call_id=1,emotion='消极',interruption=True,session_id='1',topic='现金业务',seven_type_analyzer="辱骂: True",necessary_keypoint_analyzer="a: True, b: True, c: True")
    ...
    h=ESSearch.search_dialog_by_text_5({'AND':['圆梦金','储蓄卡'],'OR':['分期'],'NOT':['现金']})
    g=ESSearch.search_dialog_by_time_range('2017-01-01 00:00:00','2018-07-31 09:00:00')
    ...
    k=ESSearch.search_dialog_by_necessary_keypoint_analyzer(['a','b'],'现金业务')
    print('testing search_dialog_by_necessary_keypoint_analyzer','\n',k,'\n')
