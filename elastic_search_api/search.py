...
class ESSearch:
    ...
    def search_dialog_by_call_id(self, call_id, page=1, size=10):
        """
        Description: 按call_id进行查找
        Params: call_id: <string>
        Params page : int,当前页数（从1开始），page大于实际总页数时返回空
        Param size : int,每页显示的条目数

        Return: {
            "result": [{'callee_no': '98076', 'machine_score': 0, 'updatetime': '2018-09-15 02:33:13', '@version': '1', 'caller_no': '14213', 'transcripts': 'pass', 'is_manual_rated': False, 'end_time': '2018-07-31 08:02:08', 'begin_time': '2018-07-31 07:59:07', 'silence': 0.0, 'manual_rating': '', 'id': 'f0ada676-b7b4-11e8-b833-3c404f107c90', '@timestamp': '2018-09-15 04:33:00', 'call_id': '1520948153-263390', 'emotion': '', 'created_at': '2018-09-07 12:41:03', 'interruption': '', 'manual_score': 0, 'status': 3, 'session_id': '80f705de-a1e0-11e8-a9a0-784f437148a2'},{}...]
            "page": int,  # 当前返回页数
            "page_nums": int # 总页数
        }
        若无符合条件的结果则result字段返回空列表
        """
        body = {'query': {'match_phrase': {'call_id': {'query': call_id}}},  # 从第几条开始获取
                "from": (page - 1) * size,
                "size": size}
        res = self.es.search(index=self.index, body=body)
        return_list = []
        ...
        total_nums = res["hits"]["total"]                      # 获取查询结果的总条数
        if (total_nums % size) > 0:                                         # 计算页数
            page_nums = int(total_nums/size)+1
        else:
            page_nums = int(total_nums/size)
        return {"result": return_list, "page": page, "page_nums": page_nums}

    def search_dialog_by_session_id(self, session_id, page=1, size=10):
        """
        ...
        若无符合条件的结果则result字段返回空列表
        """
        body = {'query': {'match_phrase': {'session_id': {'query': session_id}}}, "from": (page - 1) * size,
                "size": size}
        res = self.es.search(index=self.index, body=body)
        return_list = []
        for i in res['hits']['hits']:
    ...
    def search_dialog_by_topic(self, topic, page=1, size=10):
        """
        ...
        Param size : int,每页显示的条目数
        return : {
            "result":[{"dialog_id": <string>, "transcripts", <string>, "topic": <string>}],
            "page": int,
            "page_nums": int
        }
        若无符合条件的结果则result字段返回空列表
        """
        body = {
                    'query': {'match_phrase': {'topic': {'query': topic}}}, 
                    "from": (page-1)*size,                                # 从第几条开始获取
                    "size": size
            }
        res = self.es.search(index=self.index, body=body)
        return_list = []
    ...
    def search_dialog_by_silence(self, silence_total=None, silence_max=None, page=1, size=10):
        """
        ...
        Param size : int,每页显示的条目数
        return : {
            "result":[{"dialog_id":<string>, "transcripts": <string>, "silence_total":<float>, "silence_max":<float>}],
            "page": int,
            "page_nums": int
        }
        若无符合条件的结果则result字段返回空列表
        ...
        if silence_total and silence_max:
            ...
            body = {"query": {"range": {'silence_max': {"gte": silence_max}}}, "from": (page-1)*size,                                # 从第几条开始获取
                    "size": size}
        elif not silence_max and silence_total:
            body = {"query": {"range": {'silence_total': {"gte": silence_total}}}, "from": (page-1)*size,                                # 从第几条开始获取
                    "size": size}
        res = self.es.search(index=self.index, body=body)
        return_list = []
        return_dict = {}
    ...
    def search_dialog_by_time_range(self, start_time, end_time, page=1, size=10):
        """
        Description: 返回符合时间要求的索引内容
        Params: start_time,end_time:<string>  例:'2018-09-15 01:01:01'
        Params page : int,当前页数（从1开始），page大于实际总页数时返回空
        Param size : int,每页显示的条目数
        Return:{
            "result":[{'callee_no': '98076', 'machine_score': 0, 'updatetime': '2018-09-15 02:33:13', 'caller_no': '14213', 'transcripts': 'pass', 'is_manual_rated': False, 'end_time': '2018-07-31 08:02:08', 'begin_time': '2018-07-31 07:59:07', 'silence': 0.0, 'manual_rating': '', 'id': 'f0ada676-b7b4-11e8-b833-3c404f107c90', '@timestamp': '2018-09-15 04:33:00', 'call_id': '1520948153-263390', 'emotion': '', 'created_at': '2018-09-07 12:41:03', 'interruption': '', 'manual_score': 0, 'status': 3, 'session_id': '80f705de-a1e0-11e8-a9a0-784f437148a2'},{}...]
            "page": int,
            "page_nums": int
        }
        若无符合条件的结果则result字段返回空列表
        ...
        body = {"query": {"bool": {"must":
                                   [{"range": {'begin_time': {"gt": start_time}}},
                                    {"range": {'end_time': {"lt": end_time}}}]}}, 
                "from": (page-1)*size,                                # 从第几条开始获取
                "size": size}
        res = self.es.search(index=self.index, body=body)
        return_list = []
    ...
    def search_dialog_by_transcripts_single(self, sentence, mode="fuzzy", page=1, size=10):
        """
        Description:返回模糊匹配到的对话以及高亮部分
        Params:sentence:<string> 
        param mode:'fuzzy',''accurate'分别代表模糊、精准匹配
        Params page : int,当前页数（从1开始），page大于实际总页数时返回空
        Param size : int,每页显示的条目数
        Return:{
            "result":[{"dialog_id": <string>,'transcripts':<string>,'highlight':str},{}...],
            "page": int,
            "page_nums": int
        }
        highlight字段中高亮匹配到的部分以#开始，以@结束
        若无符合条件的结果则result字段返回空列表
        """
        if mode == "fuzzy":
            body = {
        ...
        for i in res['hits']['hits']:
            return_dict["dialog_id"]=i['_source']['dialog_id']
            return_dict["transcripts"] = i['_source']['transcripts']
            return_dict["highlight"] = str(i['highlight'])
            return_list.append(return_dict)
    ...
    def search_dialog_by_transcripts_multiple(self, sentence, logic="and", mode="fuzzy", page=1, size=10):
        '''
    ...
    def search_dialog_by_transcripts_complex_logic(self, sentence, page=1, size=10):
        '''
        Description:按逻辑’或‘和’与‘进行组合精确查询
        Params:sentence:{'AND':["word", "word"],'OR':[],'NOT':[]}  
        Params page : int,当前页数（从1开始），page大于实际总页数时返回空
        Param size : int,每页显示的条目数
        Return:{
            "result: [{"dialog_id": <string>,'transcripts':<string>,'highlight':dict},{}...],
            "page": int,
            "page_nums": int
        }
        highlight字段中高亮匹配到的部分以#开始，以@结束
    ...
    def search_dialog_by_interruption(self, interruption=True, page=1, size=10):
        '''
        descreption:检索interruption字段
        params: interruption：bool, True/False
        Params page : int,当前页数（从1开始），page大于实际总页数时返回空
        Param size : int,每页显示的条目数
        return: {
            "result":[{"dialog_id":<str>,"transcripts":<str>,"interruption":bool}],
            "page": int,
            "page_nums": int
        }
        '''
        body = {'query': {'match_phrase': {'interruption': {'query': 1 if interruption else 0}}}, 
                "from": (page-1)*size,                                # 从第几条开始获取
                "size": size}
        res = self.es.search(index=self.index, body=body)
        return_list = []
    ...
    def search_dialog_by_smart_text_analyzer(self, smart_text_analyzer, page=1, size=10):
        '''
        descreption:检索smart_text_analyzer字段，即’辱骂‘，’用心超越希望‘等七类
        params: smart_text_analyzer : str "辱骂指责"
        Params page : int,当前页数（从1开始），page大于实际总页数时返回空
        Param size : int,每页显示的条目数
        return: {
            "result":[{"dialog_id":<str>,"transcripts",<str>,"smart_text_analyzer": '{"analyzer_name1": "True", "analyzer_name2": "False"}'}]
            "page": int,
            "page_nums": int
        }
        '''
        body = {'query': {'match_phrase': {'smart_text_analyzer': {'query': smart_text_analyzer + ': True'}}}, 
                "from": (page-1)*size,                                # 从第几条开始获取
                "size": size}
        res = self.es.search(index=self.index, body=body)
        return_list = []
    ...
    def search_dialog_by_keypoint_analyzer(self, keypoint_list, topic, page=1, size=10):
        '''
        ...
        return: {
            "result":[{"dialog_id":dialog_id,"transcripts",transcripts,"keypoint": <str>(e.g. 'a: True, b: True, c: True')}],
            "page": int,
            "page_nums": int
        }
        '''
        ...
        for i in keypoint_list:
            keypoint.append({'match_phrase': {'keypoint_analyzer': {'query':i}}})  # TODO 查询错误，返回结果变少
        ...
        for i in res['hits']['hits']:
            return_dict["dialog_id"] = i['_source']['dialog_id']
            return_dict["transcripts"] = i['_source']['transcripts']
            return_dict["keypoint"] = i['_source']['keypoint_analyzer']
            return_list.append(return_dict)
    ...
    def insertES(self, dialog_id, call_id=None, session_id=None, transcripts=None, begin_time=None, end_time=None, 
        ...
        '''
        descreption:新建或者更新索引条目，每一个条目对应一个对话的所有相关信息，包括id类、时间类、分析器类等
        ...
        if len(res['hits']['hits']) == 0:  # 新建索引项
            ...
            new_res = self.es.index(index=self.index, doc_type=self.doc_type, body=body, params={"refresh":'true'})
            if new_res['result']=='created':
                return {"status": True, "message": "创建成功"}
            else:
            ...
            if keypoint_analyzer:
                body['keypoint_analyzer'] = json.dumps(keypoint_analyzer, ensure_ascii=False)   # <string>
            res = self.es.update(index=self.index, doc_type=self.doc_type, id=id_value, body={"doc": body}, params={"refresh":'true'})
            if res['result']=='noop':
                return {"status": True, "message": "更新值与原值相同"}
...
if __name__ == '__main__':
    ...
    print("查询dialog_id:", ESSearch.search_dialog_by_dialog_id(dialog_id="938ee1f4-bd3a-11e8-8106-3c404f107c90"), '\n')
    print("查询call_id:", ESSearch.search_dialog_by_call_id(call_id="1520063465-201027"), '\n')
    print("查询session_id:", ESSearch.search_dialog_by_session_id(session_id="80f705de-a1e0-11e8-a9a0-784f437148a2"), '\n')
    print("查询topic:", ESSearch.search_dialog_by_topic(topic="分期提前缴款", page=1, size=10), '\n')
    print("查询silence:", ESSearch.search_dialog_by_silence(silence_total=50, silence_max=5), '\n')
    print("查询时间范围:", ESSearch.search_dialog_by_time_range(start_time="2018-07-31 11:21:14", end_time="2018-08-31 11:21:14"), '\n')
    print("文本单句搜索:", ESSearch.search_dialog_by_transcripts_single(sentence="圆梦金", mode="fuzzy"), '\n')
    print("文本多句搜索:", ESSearch.search_dialog_by_transcripts_multiple(sentence=["圆梦金"], mode="fuzzy", logic="and"), '\n')
    print("文本复杂逻辑精准搜索:", ESSearch.search_dialog_by_transcripts_complex_logic(sentence={'AND':[],'OR':[],'NOT':[]}), '\n')
    print("查询interruption:", ESSearch.search_dialog_by_interruption(interruption=True), '\n')
    print("查询文本分析器：", ESSearch.search_dialog_by_smart_text_analyzer("用心超越期望"), '\n')
    print("查询话术分析器:", len(ESSearch.search_dialog_by_keypoint_analyzer(keypoint_list=["关键点1"], topic="分期提前缴款", page=10)["result"]), '\n')
    exit()
